# firebase_layout.md — Firebase 프로젝트 구조 (초안 v0.10)

> **상태**: 초안. Codex 감사 9차 반영 완료 (커밋 `5bdc013` 재감사 → 이 문서). 사용자 역할 구조 + audit_log 위험 수용 + Gmail API 활성화 확인 후 v1.0 승격 예정.
...
3. **분리된 감사 기록기 (선택 (b) 시 v1.0 포함, 선택 (a) 시 v1.1)** — 격리가 아니라 **감사 대상 표면적 축소**:
   ...
   - **인증 컨텍스트 손실** — Pub/Sub 트리거로 부르면 원 Firebase callable 의 auth 컨텍스트(actor 이메일·역할·요청 ID 등) 가 **전달되지 않음**. 기록기가 이벤트 페이로드의 `actor` 를 곧이곧대로 믿으면 발행자가 임의 actor 로 위조 가능. **선택 (b) 를 구현하려면 다음이 필요**:
     - **서명된 이벤트 스키마** — 발행자가 이벤트에 `{actor, action, target, request_id, timestamp}` 를 넣고, Cloud Functions 의 **서버 생성 메타데이터**(호출 시점의 authenticated identity)를 **함께 서명**. 기록기는 서명을 검증하고 서명 안의 actor 만 신뢰. 서명 키는 별도 관리 (KMS 또는 Firebase Secret Manager).
     - **또는 서버 신원 토큰 검증** — 발행자가 GCP 의 서비스 계정 신원 토큰을 이벤트에 첨부, 기록기가 검증. **주의**: 프로젝트 기본 서비스 계정을 모든 함수가 공유하면 임의 업무 함수도 같은 신원 토큰을 발급받을 수 있음 — 「이 이벤트는 우리 프로젝트의 함수가 발행했다」 만 보장. **함수별 전용 서비스 계정 + 기록기가 발행자 서비스 계정 이메일 allowlist + 토큰 audience/issuer 검증**이 있어야 「어느 함수가 발행했다」 까지 가려짐. 「어느 사용자를 대신해」 는 그 위에 서명 스키마가 별도 필요.
     - **중복 방지 (Pub/Sub at-least-once)** — 단순히 `request_id` 를 「본 적 있는지」로 확인하는 것으로는 부족. **`request_id` 를 audit_log 문서 ID 로 삼고** Firestore `.create()` 로 원자적으로 만들어 이미 있으면 실패 → 이 실패를 「중복 배송 무시」로 처리. 트랜잭션 재시도 충돌 규칙도 명세: 문서 create 만 시도, update 는 절대 안 함.
   - **서명 스키마의 한계 (내부 위조)**: 서명 키가 KMS 에 있어도, **키에 서명 권한이 있는 서비스 계정을 업무 함수가 쓰면 그 업무 함수가 임의 actor 로 서명한 이벤트를 만들 수 있다.** 서명은 「외부 재생」이나 「Pub/Sub 밖 위조」는 막지만 「같은 프로젝트 안의 오남용」은 안 막는다. 실제로 이걸 잡으려면:
     - 서명 권한을 **별도 서비스 계정**에 두고, 그 계정을 **호출자 신원과 결속된 채로만** 접근 (예: 발행자 함수의 서비스 계정에는 KMS 서명 권한 없이 두고, 발행자 → 별도 「서명 게이트웨이 함수」 → 서명 게이트웨이는 요청의 authenticated user info 를 검증한 뒤에만 서명). 이 게이트웨이 자체가 또 업무 함수라 완전 봉쇄는 아니지만 표면적이 더 좁아짐.
     - 또는 GCP 의 **IAM 서비스 계정 신원 토큰**을 이용 — 이 토큰은 GCP 가 발급하고 호출자 신원에 결속되어 있어 임의 발행자가 위조 못 함. 그러나 「어느 사용자를 대신해」 는 별도 서명 필요.
   - **이 층의 실질 값**: 정상 경로의 코드 표면적을 「기록기 함수 파일 1개 + 서명 게이트웨이」로 좁히는 것. 실수·회귀가 그 자리에 몰림 → 리뷰·테스트·모니터링 집중 가능. **완전한 기술적 격리는 아니다** — 남는 위험은 위 「내부 위조」이며 여전히 코드 리뷰가 최종 관문.

### 5-A. v1.0 위험 수용 결정 (사용자 결정 필요)

**결정할 것**: v1.0 을 (a) 코드 규율 기반으로 승격할 것인가, (b) Pub/Sub **표면적 축소**를 v1.0 에 포함할 것인가.

| | (a) 코드 규율 기반 v1.0 | (b) Pub/Sub 표면적 축소 포함 v1.0 |
|---|---|---|
...
**선택 (b) 시 별도 문서** — Pub/Sub 격리는 이 문서에 요약만 두고 실제 구현은 **별도 보안 설계 문서 `docs/design/audit_pubsub_isolation.md` 로 분리**. 그 문서에서 다룰 것:
- 함수별 전용 서비스 계정 정의 · 발행자 allowlist
- 서명 게이트웨이 함수 인터페이스 · 서명 형식 · 키 순환 정책
- 서버 신원 토큰 audience/issuer 검증 절차
- `request_id` 기반 idempotency 의 Firestore 원자적 create 구현
- 침입 시나리오별 방어·감지·복구 시험 시나리오
사용자가 (b) 를 선택하면 v1.0 승격 전에 이 문서를 작성·감사한다.

---

**v1.0 에서 실제로 도는 것 = (1) + (2)**. (3) 은 (b) 선택 시 v1.0, (a) 선택 시 v1.1. 이 자리에 「(1)+(2) 만으로 기술적으로 완벽」 이라고 안 씀 — 실질 완화는 (2) 의 관문 겹침과 코드 리뷰이며, (3) 은 그 위에 **정상 경로의 코드 표면적 축소**를 얹을 뿐이다. **서명 키 접근 통제·서비스 계정 분리·idempotency 를 다 해도 업무 함수의 Admin SDK audit_log 수정 권한 자체는 여전히 열려 있음** — 그건 IAM 의 컬렉션 단위 제한 부재라는 근본 한계에서 온다. 즉 (3) 은 「기술적 격리」가 아니라 「정상 경로 축소 + 이벤트 위조 완화」의 조합이다.
