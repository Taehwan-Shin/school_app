# firebase_layout.md — Firebase 프로젝트 구조 (초안 v0.8)

> **상태**: 초안. Codex 감사 7차 반영 완료 (커밋 `dc4f360` 재감사 → 이 문서). 사용자 역할 구조 + audit_log 위험 수용 + Gmail API 활성화 확인 후 v1.0 승격 예정.
> 인증 모델 = ⓑ 로그인 사용자 OAuth. 서비스 계정·도메인 위임 없음. 자세한 이유는 `AGENTS.md` §1.
>
> **v0.8 변경점** (감사 7차): (1) §5-(3) 제목을 조건부 (「선택 (b) 시 v1.0, 아니면 v1.1」) 로 정정. (2) §5-(3) 에 **Pub/Sub 인증 컨텍스트 손실** 명시 — 트리거로 부르면 원 callable auth 컨텍스트가 안 넘어감. (b) 를 구현하려면 서명된 이벤트 스키마 · 서버 신원 토큰 검증 · 중복 방지 (idempotency) 가 필요함을 명시.
> **v0.7 변경점** (감사 6차): §5-(3) 표면적 축소 정직화 · §5-A (b) 위험 문장 재작성.
...
3. **분리된 감사 기록기 (선택 (b) 시 v1.0 포함, 선택 (a) 시 v1.1)** — 격리가 아니라 **감사 대상 표면적 축소**:
   ...
   - Pub/Sub 발행자 자체가 위조 이벤트를 낼 위험은 위 서명 스키마로 완화된다.
   - **이 층의 실질 값**: 정상 경로의 코드 표면적을 「기록기 함수 파일 1개 + 서명 스키마」로 좁히는 것. 실수·회귀가 그 자리에 몰림 → 리뷰·테스트·모니터링 집중 가능. **완전한 격리는 아니다.**
