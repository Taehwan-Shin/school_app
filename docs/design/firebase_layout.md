# firebase_layout.md — Firebase 프로젝트 구조 (초안 v0.9)

> **상태**: 초안. Codex 감사 8차 반영 완료 (커밋 `e8f8695` 재감사 → 이 문서). 사용자 역할 구조 + audit_log 위험 수용 + Gmail API 활성화 확인 후 v1.0 승격 예정.
> 인증 모델 = ⓑ 로그인 사용자 OAuth. 서비스 계정·도메인 위임 없음. 자세한 이유는 `AGENTS.md` §1.
>
> **v0.9 변경점** (감사 8차): (1) §5-(3) 서명 스키마의 「내부 위조」 한계 명시 — 서명 키 접근 권한이 있는 발행자는 임의 actor 로 서명 가능, 완화하려면 서명 게이트웨이 함수 또는 GCP 신원 토큰 결속 필요. (2) §5-A 결정문 · 표 제목 · §5 하단 결론 · §7-미결 §4 에서 「격리」 용어 삭제 → **표면적 축소**로 통일. (3) §7-미결 §4 에 조건부 일정 (「선택 (b) 시 v1.0, 선택 (a) 시 v1.1」) 추가.
> **v0.8 변경점** (감사 7차): §5-(3) 조건부 제목 · Pub/Sub 인증 컨텍스트 손실 · 서명 스키마 요구.
...
3. **분리된 감사 기록기 (선택 (b) 시 v1.0 포함, 선택 (a) 시 v1.1)** — 격리가 아니라 **감사 대상 표면적 축소**:
   ...
   - **인증 컨텍스트 손실** — Pub/Sub 트리거로 부르면 원 Firebase callable 의 auth 컨텍스트(actor 이메일·역할·요청 ID 등) 가 **전달되지 않음**. 기록기가 이벤트 페이로드의 `actor` 를 곧이곧대로 믿으면 발행자가 임의 actor 로 위조 가능. **선택 (b) 를 구현하려면 다음이 필요**:
     ...
     - **또는 서버 신원 토큰 검증** — 발행자가 GCP 의 서비스 계정 신원 토큰을 이벤트에 첨부, 기록기가 검증. 이 방식은 「이 이벤트는 우리 프로젝트의 함수가 발행했다」 만 보증하고 「어느 사용자를 대신해」 는 별도 필요.
     - **중복 방지** — `request_id` 를 이벤트 식별자로 삼고 기록기는 같은 `request_id` 를 두 번 안 씀. Pub/Sub 이 at-least-once 배송이므로 idempotency 필수.
...
**v1.0 에서 실제로 도는 것 = (1) + (2)**. (3) 은 (b) 선택 시 v1.0, (a) 선택 시 v1.1. 이 자리에 「(1)+(2) 만으로 기술적으로 완벽」 이라고 안 씀 — 실질 완화는 (2) 의 관문 겹침과 코드 리뷰이며, (3) 은 그 위에 **정상 경로의 코드 표면적 축소**를 얹을 뿐이다 (기술적 격리는 서명 키 접근 통제까지 갔을 때만 성립하며, 그것도 완전 봉쇄는 아님).
