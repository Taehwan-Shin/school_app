...
- **`useLocalStorageState` shared hook** - v0.219 완료 (인프라). React state + localStorage 자동 동기화 · SetStateAction<T> · serialize/deserialize ref pattern · key 변경 재 hydrate · writeVersion → pending snapshot 3-tuple · StrictMode safe. Codex 6 라운드 iteration (R0-R6). Main HEAD `caaf998`.
- **4 테이블 pageSize hook 이식** - v0.220 완료. Shared `pageSizeStorage.ts` factory · 4 테이블 이식 · Codex R2 통과 (13/0/0) · 웹 1242.
