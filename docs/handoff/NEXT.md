...
> **지금 이 파일에 열린 오더 없음.** v0.105 (SuperAdminPage role_split 감시 카드) 병합 완료 (`ef6fb16`).
> v0.104 (audit multi-action filter) 는 별 브랜치 `feat/audit-multi-action-v104 @ 470d854` Codex 감사 응답 지연 대기 중.
...
STATUS.md 참조:
- **v0.104 재요청** — audit 다중 액션 필터 Codex 감사 5시간+ 무응답. 재감사 요청 또는 병합 재검토.
- **server-side role_split 전용 action** — v0.105 client filter 불필요화. `role.split_detected` 등 신규 action 도입 · `readAudit` 서버 필터.
