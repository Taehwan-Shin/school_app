...
export const classroomStudentsAdd = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<ClassroomStudentsAddResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<ClassroomStudentsAddRequest> | undefined;
    ...
    const target =
      courseIdPart === '*' && userIdPart === '*'
        ? '*'
        : `courses/${courseIdPart}/students/${userIdPart}`;
    ...
    try {
      ...
      await writeAudit({
        ...
        role: user.role,
        action: 'classroom.students.add',
        target: `courses/${courseId}/students/${userId}`,
        request_id: requestId,
        result: 'ok',
