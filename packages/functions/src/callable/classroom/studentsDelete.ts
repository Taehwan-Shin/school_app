...
export const classroomStudentsDelete = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<ClassroomStudentsDeleteResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<ClassroomStudentsDeleteRequest> | undefined;
