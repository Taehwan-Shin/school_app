...
export const classroomTeachersAdd = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<ClassroomTeachersAddResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<ClassroomTeachersAddRequest> | undefined;
