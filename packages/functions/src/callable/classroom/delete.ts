...
export const classroomDelete = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<ClassroomDeleteResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<ClassroomDeleteRequest> | undefined;
    const targetId =
    ...
    try {
      ...
      await classroom.courses.delete({ id });
      ...
      await writeAudit({
        ...
        request_id: requestId,
        result: 'ok',
        message: `deleted classroom course ${id}`,
      });
