...
import { getClassroomClient, type ClassroomStudent } from '../../google/classroomClient.js';
...
export const classroomStudentsList = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<ClassroomStudentsListResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<ClassroomStudentsListRequest> | undefined;
    const targetId =
      typeof data?.courseId === 'string' && data.courseId.trim() ? data.courseId.trim() : '*';
    const target = targetId === '*' ? '*' : `courses/${targetId}/students`;
    ...
    try {
      ...
      const results: ClassroomStudent[] = [];
      ...
      do {
        ...
        results.push(...(res.data.students ?? []));
      ...
      await writeAudit({
        ...
        role: user.role,
        action: 'classroom.read',
        target: `courses/${courseId}/students`,
        request_id: requestId,
        result: 'ok',
        message: `listed ${results.length} students for course ${courseId}`,
      });
