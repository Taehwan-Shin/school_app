...
export const groupsDelete = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<GroupsDeleteResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<GroupsDeleteRequest> | undefined;
    const targetEmail =
    ...
    try {
      ...
      await directory.groups.delete({ groupKey: trimmedEmail });
