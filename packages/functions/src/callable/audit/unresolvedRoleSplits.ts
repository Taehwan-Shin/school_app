...
// - resolved: result='ok' 만 인정 (denied/error 는 실패한 복구 시도이므로 상태 전이 아님).
// - unresolved = detected.at > (latest resolved.at ?? -Infinity).
//
...
export const auditLogUnresolvedRoleSplits = onCall(
  ...
  async (request): Promise<AuditLogUnresolvedRoleSplitsResponse> => {
    ...
    try {
      ...
      const [detectedRes, resolvedRes] = await Promise.all([
        readAuditEntries({
          limit: scanLimit,
          atMin,
      ...
      for (const e of resolvedRes.entries) {
        const prev = latestResolvedAtByTarget.get(e.target) ?? -Infinity;
        if (e.at > prev) latestResolvedAtByTarget.set(e.target, e.at);
      }
      ...
      await writeAudit({
        ...
        request_id: requestId,
        result: 'ok',
        message: `unresolved=${unresolved.length} scannedDetected=${detectedRes.entries.length} scannedResolved=${resolvedRes.entries.length}`,
      });

      return {
        entries: unresolved,
        scannedDetected: detectedRes.entries.length,
        scannedResolved: resolvedRes.entries.length,
        detectedHasMore: detectedRes.nextCursor !== null,
        resolvedHasMore: resolvedRes.nextCursor !== null,
      };
    } catch (err) {
      const mapped =
