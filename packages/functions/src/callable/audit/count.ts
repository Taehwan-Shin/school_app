...
export const auditLogCount = onCall(
  { region: 'asia-northeast3', cors: true },
  async (request): Promise<AuditLogCountResponse> => {
    const requestId = readHeader(request, 'x-request-id') ?? crypto.randomUUID();
    const data = request.data as Partial<AuditLogCountRequest> | undefined;
    ...
    try {
      ...
      await writeAudit({
        ...
        request_id: requestId,
        result: 'ok',
        message: `counted ${count} entries${filterStr}`,
      });
