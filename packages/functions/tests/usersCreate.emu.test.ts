...
describe("usersCreate integration (Auth + Firestore + Functions Emulator)", () => {
  ...
  it("runs REST signUp -> promote to admin -> signIn -> call usersCreate -> writes ok audit_log to Firestore Emulator", async () => {
    const adminEmail = `admin-create-emu-${Date.now()}@cam.hs.kr`;
    ...
    const newStudentEmail = "student1@cam.hs.kr";
    writeStub({
      data: {
        insert: {
          id: "google-uid-1001",
          primaryEmail: newStudentEmail,
        },
      },
    });

    // 5. Call HTTP endpoint with admin idToken
    const requestId = `req-create-http-${Date.now()}`;
    const res = await fetch(`http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/usersCreate`, {
      method: "POST",
      headers: {
    ...
    expect(auditDoc.target).toBe(newStudentEmail);
    expect(auditDoc.result).toBe("ok");
    expect(auditDoc.message).toBe("created user");
  });
});
