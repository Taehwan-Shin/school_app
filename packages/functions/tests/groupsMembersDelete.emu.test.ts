...
describe('groupsMembersDelete integration (Auth + Firestore + Functions Emulator)', () => {
  ...
  it('runs REST signUp -> promote to admin -> signIn -> call groupsMembersDelete -> writes ok audit_log to Firestore Emulator', async () => {
    const email = `admin-emu-members-del-${Date.now()}@cam.hs.kr`;
    ...
    const requestId = `req-admin-members-del-http-${Date.now()}`;
    const res = await fetch(
      `http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/groupsMembersDelete`,
      {
        method: 'POST',
        headers: {
    ...
    expect(auditDoc.message).toBe('removed leave@cam.hs.kr from group team-a@cam.hs.kr');
  });

  it('rejects non-admin (teacher) via HTTP endpoint and writes denied audit_log', async () => {
    const email = `teacher-emu-members-del-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const requestId = `req-teacher-members-del-http-${Date.now()}`;
  ...
  it('rejects admin with missing group.member scope and writes denied audit_log', async () => {
    const email = `admin-noscope-members-del-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const requestId = `req-noscope-members-del-http-${Date.now()}`;
