...
describe('groupsMembersList integration (Auth + Firestore + Functions Emulator)', () => {
  ...
  it('runs REST signUp -> promote to admin -> signIn -> call groupsMembersList -> writes ok audit_log to Firestore Emulator', async () => {
    const email = `admin-emu-members-list-${Date.now()}@cam.hs.kr`;
    ...
    const requestId = `req-admin-members-list-http-${Date.now()}`;
    const res = await fetch(
      `http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/groupsMembersList`,
      {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({ data: { groupEmail: 'team-a@cam.hs.kr' } }),
      },
    );
    ...
    expect(auditDoc.action).toBe('groups.read');
    expect(auditDoc.target).toBe('team-a@cam.hs.kr');
    expect(auditDoc.result).toBe('ok');
    expect(auditDoc.message).toBe('listed 2 members of group team-a@cam.hs.kr');
  });

  it('rejects non-admin (teacher) via HTTP endpoint and writes denied audit_log', async () => {
    const email = `teacher-emu-members-list-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const requestId = `req-teacher-members-list-http-${Date.now()}`;
  ...
  it('rejects admin with missing group.member.readonly scope and writes denied audit_log', async () => {
    const email = `admin-noscope-members-list-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const requestId = `req-noscope-members-list-http-${Date.now()}`;

    // 4. Call HTTP endpoint without member.readonly scope
    const res = await fetch(
      `http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/groupsMembersList`,
      {
        method: 'POST',
