...
describe('groupsList integration (Auth + Firestore + Functions Emulator)', () => {
  ...
  it('runs REST signUp -> promote to admin -> signIn -> call groupsList -> writes ok audit_log to Firestore Emulator', async () => {
    const email = `admin-emu-groups-${Date.now()}@cam.hs.kr`;
    ...
    writeStub({
      data: {
        groups: [
          ...
          {
            email: 'team-b@cam.hs.kr',
            name: 'Team B',
            description: '',
            aliases: ['tb@cam.hs.kr'],
    ...
    const requestId = `req-admin-groups-http-${Date.now()}`;
    const res = await fetch(`http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/groupsList`, {
      method: 'POST',
      headers: {
    ...
    expect(auditDoc.message).toBe('listed 2 groups');
  });

  it('rejects non-admin (teacher) via HTTP endpoint and writes denied audit_log', async () => {
    const email = `teacher-emu-groups-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const requestId = `req-teacher-groups-http-${Date.now()}`;
  ...
  it('rejects admin with missing group scope and writes denied audit_log', async () => {
    const email = `admin-noscope-groups-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const requestId = `req-noscope-groups-http-${Date.now()}`;

    // 4. Call HTTP endpoint without group scope header (or only user scope)
    const res = await fetch(`http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/groupsList`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
