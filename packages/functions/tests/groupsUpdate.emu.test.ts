...
describe('groupsUpdate integration (Auth + Firestore + Functions Emulator)', () => {
  ...
  it('runs REST signUp -> promote to admin -> signIn -> call groupsUpdate -> writes ok audit_log with before/after to Firestore Emulator', async () => {
    const adminEmail = `admin-update-group-${Date.now()}@cam.hs.kr`;
    ...
    const targetGroupEmail = 'target-team@cam.hs.kr';
    writeStub({
      data: {
        groupGet: {
          id: 'group-target-id',
    ...
    const requestId = `req-update-group-http-${Date.now()}`;
    const res = await fetch(`http://${FUNCTIONS_HOST}/${PROJECT_ID}/asia-northeast3/groupsUpdate`, {
      method: 'POST',
      headers: {
    ...
    expect(auditDoc.message).toContain('updated fields: name, description');
    expect(auditDoc.message).toContain('before: {"name":"Original Team","description":"Original description"}');
    expect(auditDoc.message).toContain('after: {"name":"Modified Team","description":"Modified description"}');
  });

  // 2. denied non-admin (teacher) — 403
  it('rejects non-admin (teacher) via HTTP endpoint and writes denied audit_log', async () => {
    const teacherEmail = `teacher-update-group-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const targetEmail = 'target-team@cam.hs.kr';
    const requestId = `req-teacher-update-group-http-${Date.now()}`;
  ...
  it('rejects admin with missing group write scope and writes denied audit_log', async () => {
    const adminEmail = `admin-noscope-update-${Date.now()}@cam.hs.kr`;
    const password = 'test-password-123';
    ...
    const targetEmail = 'target-team@cam.hs.kr';
    const requestId = `req-noscope-update-group-http-${Date.now()}`;
