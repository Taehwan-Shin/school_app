...
function ChatBulkInviteDialogContent({
  open,
  onOpenChange,
  spaceName,
  spaceDisplayName,
  onDone,
}: ChatBulkInviteDialogProps) {
  ...
  // v0.141: rosters 는 하위 useMemo 의 dep 로 쓰여 안정 참조 필요 (falsy path
  // 매 렌더 새 객체). v0.140 lint 도입 warning fix.
  ...
  const handleExecute = async () => {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      try {
        await callChatMembersAdd({ spaceName, email });
    ...
    setPhase('done');
    queryClient?.invalidateQueries({ queryKey: ['chat', 'members', spaceName] });
  };
      ...
      <DialogContent className={phase === 'running' ? '[&>button]:hidden max-w-2xl' : 'max-w-2xl'}>
        {phase === 'select' && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                <span className="font-mono">{spaceDisplayName || spaceName}</span> 챗방에 학급 학생들을 일괄 초대합니다.
              </DialogDescription>
            </DialogHeader>

            <div className="space-y-4 my-2">
              <div className="flex items-center gap-2">
                <label htmlFor="chat-bulk-invite-year-input" className="text-small text-fg-secondary">
                  연도:
                </label>
                <input
                  id="chat-bulk-invite-year-input"
                  type="number"
                  min={1900}
                  max={2200}
                  value={yearInput}
                  onChange={(e) => handleYearChange(e.target.value)}
                  data-testid="chat-bulk-invite-year-input"
              ...
              {basicDataQuery.isLoading && (
                <div className="py-6 text-center text-small text-fg-secondary" data-testid="chat-bulk-invite-loading">
                  기초 데이터 로딩 중...
                </div>
              )}

              {basicDataQuery.isError && (
                <div className="border border-state-danger p-3 text-small text-state-danger" data-testid="chat-bulk-invite-error">
                          ...
                          {g.classes.map((c) => {
                            ...
                            return (
                              <Button
                                ...
                                data-testid={`chat-bulk-invite-class-btn-${g.grade}-${c}`}
                              >
                                {c}반
                              </Button>
                            );
              ...
              {selectedGrade !== null && selectedClass !== null && (
                <div className="space-y-2" data-testid="chat-bulk-invite-preview">
            ...
            <DialogFooter>
              ...
              <Button
                data-testid="chat-bulk-invite-preview-btn"
                disabled={
        ...
        {phase === 'preview' && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                <span className="font-mono">{spaceDisplayName || spaceName}</span> 챗방에 {selectedGrade}학년 {selectedClass}반 학생 {targets.length}명을 초대합니다.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 my-2">
              ...
              <div className="space-y-1">
                <label htmlFor="chat-bulk-invite-confirm-input" className="text-small text-fg-primary block">
                  확인을 위해 대상 학생 수 (<strong>{targets.length}</strong>)를 입력하세요:
                </label>
                <input
                  id="chat-bulk-invite-confirm-input"
                  type="text"
                  value={confirmText}
                  onChange={(e) => setConfirmText(e.target.value)}
                  placeholder={String(targets.length)}
                  data-testid="chat-bulk-invite-confirm-input"
            ...
            <DialogFooter>
              ...
              <Button
                ...
                data-testid="chat-bulk-invite-execute-btn"
              >
                초대 실행
              </Button>
            </DialogFooter>
        ...
        {phase === 'running' && (
          <>
            ...
            <div className="py-8 text-center space-y-3" data-testid="chat-bulk-invite-running">
            ...
            <div data-testid="chat-bulk-invite-done" className="space-y-3">
              ...
              {results.some((r) => r.kind === 'skipped') && (
                <ul
                  ...
                  data-testid="chat-bulk-invite-skipped"
                >
                  {results
...
export function ChatBulkInviteDialog(props: ChatBulkInviteDialogProps) {
  if (!props.open) return null;
  return <ChatBulkInviteDialogContent {...props} />;
}
