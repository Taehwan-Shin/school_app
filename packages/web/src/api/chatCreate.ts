...
export async function callChatCreate(data: ChatCreateRequest): Promise<ChatCreateResponse> {
  ...
  const url = import.meta.env.DEV
    ? `http://127.0.0.1:5001/${projectId}/asia-northeast3/chatCreate`
    : `https://asia-northeast3-${projectId}.cloudfunctions.net/chatCreate`;
  ...
  return (body.result ?? body) as ChatCreateResponse;
}
