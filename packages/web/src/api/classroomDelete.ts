...
export async function callClassroomDelete(data: ClassroomDeleteRequest): Promise<ClassroomDeleteResponse> {
  ...
  const url = import.meta.env.DEV
    ? `http://127.0.0.1:5001/${projectId}/asia-northeast3/classroomDelete`
    : `https://asia-northeast3-${projectId}.cloudfunctions.net/classroomDelete`;
  ...
  return (body.result ?? body) as ClassroomDeleteResponse;
}
