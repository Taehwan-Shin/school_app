...
export async function callChatDelete(data: ChatDeleteRequest): Promise<ChatDeleteResponse> {
  ...
  const url = import.meta.env.DEV
    ? `http://127.0.0.1:5001/${projectId}/asia-northeast3/chatDelete`
    : `https://asia-northeast3-${projectId}.cloudfunctions.net/chatDelete`;
  ...
  return (body.result ?? body) as ChatDeleteResponse;
}
