...
export async function callOrgunitsCreate(
  ...
  const url = import.meta.env.DEV
    ? `http://127.0.0.1:5001/${projectId}/asia-northeast3/orgunitsCreate`
    : `https://asia-northeast3-${projectId}.cloudfunctions.net/orgunitsCreate`;
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      'X-Google-Scopes': 'https://www.googleapis.com/auth/admin.directory.orgunit',
      'X-Request-Id': requestId,
    },
    body: JSON.stringify({ data: { ...input, _googleAccessToken: googleAccessToken } }),
  });
  ...
  return (body.result ?? body) as OrgunitsCreateResponse;
}
