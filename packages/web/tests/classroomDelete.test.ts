...
import { callClassroomDelete, useClassroomDelete } from '../src/api/classroomDelete';
...
describe('classroomDelete API & Hook', () => {
  ...
  // 시나리오 1: 200 응답 -> data.ok
  it('scenario 1: 200 response returns ok true and invalidates classroom list query', async () => {
    let capturedUrl = '';
    ...
    const { result } = renderHook(() => useClassroomDelete(), {
    ...
    expect(capturedUrl).toContain('/asia-northeast3/classroomDelete');
  ...
  it('scenario 2: 404 response causes hook to throw and reports error', async () => {
    ...
    await expect(
      result.current.mutateAsync({ id: 'c-nonexistent' }),
