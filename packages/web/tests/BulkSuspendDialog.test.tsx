...
describe("BulkSuspendDialog component", () => {
  ...
  // v0.124 (== v0.123b F99 대칭): confirm 시점의 emails snapshot 확정.
  // 실행 중 부모 selection 이 바뀌어도 running/done 은 원래 대상만 집계.
  it("v0.124 F99: confirm 시 emails snapshot 확정 · 실행 중 prop 변경 무시", async () => {
    ...
    fireEvent.change(screen.getByTestId("bulk-suspend-confirm-input"), {
      target: { value: "2" },
    });
    ...
    expect(screen.getByTestId("bulk-suspend-running").textContent).toContain("2");
    ...
    expect(screen.getByTestId("bulk-suspend-done").textContent).toContain("2명 성공");
  });

  // v0.124 (== v0.123b F100 대칭): label htmlFor 로 프로그램적 연결.
  it("v0.124 F100: confirm input 은 label htmlFor 로 프로그램적 연결", () => {
    renderWithClient(
      <BulkSuspendDialog open={true} onOpenChange={vi.fn()} emails={["u@cam.hs.kr"]} />
    );
    const input = screen.getByLabelText(/확인을 위해 대상 개수/);
    expect(input).toBeDefined();
    expect((input as HTMLInputElement).id).toBe("bulk-suspend-confirm-input");
  });
