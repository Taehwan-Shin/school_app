...
import {
  callClassroomStudentsDelete,
  useClassroomStudentsDelete,
} from '../src/api/classroomStudentsDelete';
...
describe('classroomStudentsDelete API & Hook', () => {
  ...
  it('scenario 1: 200 response returns ok true and invalidates students query', async () => {
    let capturedUrl = '';
    ...
    const { result } = renderHook(() => useClassroomStudentsDelete(), {
    ...
    expect(capturedUrl).toContain('/asia-northeast3/classroomStudentsDelete');
  ...
  it('scenario 2: 404 response causes hook to throw and reports error', async () => {
    ...
    await expect(
      result.current.mutateAsync({ courseId: 'c-101', userId: 'student-nonexistent' }),
  ...
  it('throws error when user is not authenticated', async () => {
    ...
    await expect(
      callClassroomStudentsDelete({ courseId: 'c-101', userId: 'student1@cam.hs.kr' }),
